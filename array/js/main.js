...
// 访问元素
export function random_access(nums) {
    // 在区间[0, len(nums) - 1] 中随机抽取一个数字
    const random_index = Math.floor(Math.random() * nums.length)
    console.log('随机索引为', random_index)
    // 获取并返回随机元素
 ...
 * @param {number} index
 */
export function insert(nums, num, index) {
...
// 删除元素
export function remove(nums, index) {
...
// 查找元素
export function find(nums, target) {
    /**
     * 在数组中查找元素
     */
    for (let i = 0; i < nums.length; i++) {
...
const fIndex = find(fNums, 4)
console.log(`查找的元素索引为：${fIndex}`)

// 扩容数组
export function extend(nums, enlarge) {
    /**
     * 拓展数组长度
     */
    // 初始化一个拓展长度后的数组
