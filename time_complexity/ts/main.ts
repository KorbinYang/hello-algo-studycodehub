...
export function array_traversal(list: number[]): number {
    let count = 0
...
// 指数阶O(2^n),递归实现
export function exp_recur(n: number): number {
    if (n === 1) {
        return 1
    }
...
// 对数阶O(log n)
export function logarithmetic(n: number): number {
    let count = 0
    while (n > 1) {
        n = n / 2
        count += 1
...
// 对数阶O(log n)，递归实现
export function log_recur(n: number): number {
    if (n <= 1) {
        return 0
    }
...
// 线性对数阶O(nlogn)
export function linear_log_recur(n: number): number {
    if (n <= 1) {
        return 1
    }
...
// 阶乘阶（O(n!)）
export function factorial_recur(n: number): number {
    if (n === 0) {
        return 1
    }
    let count = 0
...
// 最差、最佳、平均时间复杂度
export function random_numbers(n: number): number[] {
    const nums = []
    for (let i = 1; i <= n; i++) {
        nums.push(i)
    }
...
export function find_one(nums: number[]): number {
